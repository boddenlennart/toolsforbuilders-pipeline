...
const REEL_PATH = join(__dirname, 'instagram/data/samples/reels/reel-2026-03-05T07-16-08.mp4');
...
async function sendTikTokHandoff(videoPath) {
  ...
  const secrets = readFileSync(join(__dirname, 'instagram/.env.secrets'), 'utf8');
...
async function sendTelegram(token, videoPath) {
  ...
  const { default: FormData } = await import('/root/.openclaw/workspace/scripts/instagram/node_modules/form-data/lib/form_data.js');
